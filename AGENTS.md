...
- kaggle CLI가 만료 토큰을 자동 갱신하지 못하는 버그: `uv run kaggle auth print-access-token > ~/.kaggle/access_token && chmod 600 ~/.kaggle/access_token`으로 우회 (액세스 토큰 1h 만료 — API 401 시 이 갱신부터).

...
LETHAL_PRIZE_TRIGGER = 4  # H-029a: 완화(4) — 리썰 창 확대
LETHAL_BUDGET_SECONDS = 3.0  # H-029a: 예산 상향
LETHAL_OVERAGE_FLOOR = 200.0
DEFENSE_PRIZE_TRIGGER = 2  # H-029b: 상대 잔여 프라이즈가 이하면 위기 — 탐색 가치투표로 방어
DEFENSE_BUDGET_SECONDS = 2.0
_MAIN_CONTEXT = 0  # SelectContext.MAIN — 원시 dict 선별용 상수
_DIAG = {"probe": 0, "commit": 0, "replay": 0, "fallback": 0, "heavy": 0, "defense": 0}
...
def _raw_lethal_window(obs_dict: dict) -> bool:
    ...
    try:
        ...
        my_window = len(me.get("prize") or []) <= LETHAL_PRIZE_TRIGGER
        danger = len(opp.get("prize") or []) <= DEFENSE_PRIZE_TRIGGER
        if not (my_window or danger):
            return False
...
def agent(obs_dict: dict):
    ...
    try:
        ...
        if (
            ...
            and len(obs.select.option) > 1
            and int(obs.select.context) == int(ls.SelectContext.MAIN)
            and len(opp.prize) <= DEFENSE_PRIZE_TRIGGER
            and overage > LETHAL_OVERAGE_FLOOR
        ):
            picked = ls._search_choose(obs, DEFENSE_BUDGET_SECONDS)
            if picked is not None:
                n = len(obs.select.option)
                if (
                    obs.select.minCount <= len(picked) <= obs.select.maxCount

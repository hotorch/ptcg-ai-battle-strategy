"""Determinized turn-level search on Mega Lopunny (H-011).
...
SEARCH_BUDGET_NODES = 350
SEARCH_BUDGET_SECONDS = 1.5
GAME_SEARCH_SECONDS_CAP = 600.0  # runTimeout 방어: 초과 시 남은 경기는 fallback 정책
...
def _search_choose(obs) -> list[int] | None:
    start = _time.perf_counter()
    mi = obs.current.yourIndex
    det = _determinize(obs)
    root = search_begin(obs, *det)
    counter = itertools.count()
    best_val, best_first = float("-inf"), None
    budget = SEARCH_BUDGET_NODES
        ...
        while heap and budget > 0 and _time.perf_counter() < deadline:
            ...
            if st.result != -1 or st.yourIndex != mi or o.select is None or not o.select.option:
                ...
                if st.result == -1 and st.yourIndex != mi:
                    # 내턴 종료 가치와 상대 응수 후 가치를 섞어 지평선 비관을 완화한다.
                    replied = _greedy_reply(ss, mi, deadline=deadline + 0.5)
                    replied_state = replied.observation.current
                    if replied_state is not None:
                        value = 0.5 * value + 0.5 * _eval_state(replied_state, mi)
                if first is not None and value > best_val:
                    best_val, best_first = value, first
                continue
            cap = 8 if o.select.context == SelectContext.MAIN else 5
            for sel in _candidate_selects(o, cap):
                ...
                if child_first is not None and value > best_val:
                    best_val, best_first = value, child_first
...
FIGHTING_LINE = {673, 674, 675, 676, 677, 678}
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        if (
            ...
            and obs.search_begin_input is not None
            and _search_spent["seconds"] < GAME_SEARCH_SECONDS_CAP
            and not _fighting_matchup(obs)
        ):
            try:
                picked = _search_choose(obs)
                if picked is None:
                    _DIAG["search_none"] += 1
                else:
                    _DIAG["search_ok"] += 1

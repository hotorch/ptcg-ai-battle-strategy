"""Search + learned value function on Majkel Lucario (H-020: H-019 + 4일치 데이터 갱신).
...
H-019 추가 2종 (공개 1341 룰베이스 에이전트의 검증된 기법을 탐색 구조에 이식):
- 프라이즈 추론: 덱 전체가 보이는 관측(select.deck) 시점에 "덱리스트 − 가시
  카드 − 덱 내용 = 프라이즈"를 확정한다. 크기가 정확히 일치할 때만 커밋한다
  (진행 중 카드로 인한 오프바이원 — 틀린 추론은 추론 없음보다 나쁘다).
  이후 결정화에서 프라이즈를 고정해 탐색이 불가능한 fetch 라인을 찾는 것을 막는다.
- 리썰 커밋: 탐색이 승리 종단에 도달하면 그 행동 경로를 신선한 결정화로
  재검증하고, 2회 이상 승리가 확인되면 가치 평균 투표 대신 라인 전체를
  descriptor 기준으로 재생한다.
"""
...
# ---------------------------------------------------------------------------
# H-019: 프라이즈 추론 — 덱 전체 관측 시점에 프라이즈 확정 집합을 잠근다.
# ---------------------------------------------------------------------------
_PRIZED: dict = {"counts": None}


def _update_prize_inference(obs) -> None:
    """select.deck(덱 전체 열람) 관측 시 프라이즈를 역산한다.

    덱리스트 − 가시 카드(핸드/버림/보드/공개 프라이즈) − 덱 내용 = 숨은 프라이즈.
    잔여 크기가 숨은 프라이즈 수와 정확히 일치할 때만 커밋한다.
    """
    if obs.select is None or obs.select.deck is None:
        return
    state = obs.current
    me = state.players[state.yourIndex]
    deck_ids = [c.id for c in obs.select.deck if c is not None]
    pool = _remove_seen(DECK, _seen_ids(me, include_hand=True))
    remainder = _remove_seen(pool, deck_ids)
    hidden = sum(1 for c in me.prize if c is None)
    if hidden <= 0:
        return
    # 효과 발동 중인 카드는 어느 존에도 없어 잔여에 유령으로 남는다(오프바이원).
    # select.effect가 그 카드를 직접 알려주므로 한 장 보정한다.
    if len(remainder) == hidden + 1:
        effect = obs.select.effect
        if effect is not None and effect.id in remainder:
            remainder = list(remainder)
            remainder.remove(effect.id)
    if len(remainder) == hidden:
        import collections as _c

        _PRIZED["counts"] = dict(_c.Counter(remainder))
        _DIAG["prize_locked"] = _DIAG.get("prize_locked", 0) + 1


def _known_prized(pool: list[int], n_prize: int) -> list[int] | None:
    """잠긴 프라이즈 추론을 현재 미관측 풀과 대조해 유효하면 반환한다."""
    counts = _PRIZED.get("counts")
    if not counts or n_prize <= 0:
        return None
    import collections as _c

    pool_counts = _c.Counter(pool)
    picked: list[int] = []
    for cid, n in counts.items():
        picked += [cid] * min(n, pool_counts.get(cid, 0))
    return picked if len(picked) == n_prize else None
...
def _determinize(obs):
    ...
    prized = _known_prized(my_pool, n_prize)
    if prized is not None:
        my_prize = _pad_to(list(prized), len(me.prize), filler)
        rest = _remove_seen(my_pool, prized)
    else:
...
def _greedy_reply(ss, mi: int, step_limit: int = 30, deadline: float | None = None):
    ...
    for _ in range(step_limit):
        ...
        o = ss.observation
        st = o.current
        if st is None or _sim_winner(st, mi) != -1 or st.yourIndex == mi:
            return ss
...
# H-019: 검증된 승리 라인의 잔여 스텝. descriptor 기준으로 재생한다.
_WIN_LINE: dict = {"turn": None, "steps": None}


def _sim_winner(st, mi: int) -> int:
    """탐색 샌드박스는 result를 설정하지 않는다(실측: 2.5만 노드 terminal 0).

    상태 프록시로 승패를 판정한다: 잔여 프라이즈 소진(len(prize)==0) 또는
    상대 보드 전멸이면 승리. 반대면 패배. 둘 다 아니면 -1(진행 중).
    """
    if st.result != -1:
        return st.result
    me = st.players[mi]
    opp = st.players[1 - mi]
    me_alive = any(p is not None for p in list(me.active) + list(me.bench))
    opp_alive = any(p is not None for p in list(opp.active) + list(opp.bench))
    if len(me.prize) == 0 or not opp_alive:
        return mi
    if len(opp.prize) == 0 or not me_alive:
        return 1 - mi
    return -1


def _value_of(st, mi: int) -> float:
    winner = _sim_winner(st, mi)
    if winner == mi:
        return 1e9
    if winner == 1 - mi:
        return -1e9
    return _eval_state(st, mi)


def _match_descs(obs, descs: tuple) -> list[int] | None:
    """기록된 descriptor 목록을 현재 옵션 인덱스로 사상한다(인덱스 이동 대응)."""
    available: dict[str, list[int]] = {}
    for i, option in enumerate(obs.select.option):
        available.setdefault(describe(obs, option), []).append(i)
    picked = []
    for desc in descs:
        slots = available.get(desc)
        if not slots:
            return None
        picked.append(slots.pop(0))
    return picked


def _replay_path(ss, path, mi: int) -> bool:
    """탐색 상태에서 경로를 descriptor 매칭으로 재생, 승리 종단 도달 여부."""
    for _sel, descs in path:
        o = ss.observation
        st = o.current
        if st is None:
            return False
        ...
        if winner == mi:
            return True
        if winner != -1 or o.select is None or not o.select.option:
            return False
        indices = _match_descs(o, descs)
        if indices is None:
            return False
        try:
            ss = search_step(ss.searchId, indices)
        except Exception:  # noqa: BLE001
            return False
    st = ss.observation.current
    return st is not None and _sim_winner(st, mi) == mi


def _verify_win_line(obs, path, tries: int = 2) -> int:
    """신선한 결정화 tries개에서 승리 라인을 재검증한다."""
    mi = obs.current.yourIndex
    ok = 0
    for _ in range(tries):
        try:
            ss = search_begin(obs, *_determinize(obs))
            won = _replay_path(ss, path, mi)
        except Exception:  # noqa: BLE001
            won = False
        finally:
            try:
...
def _pop_win_step(obs) -> list[int] | None:
    """커밋된 승리 라인의 다음 스텝을 재생한다. 어긋나면 즉시 폐기하고 탐색으로."""
    steps = _WIN_LINE.get("steps")
    if not steps:
        return None
    if obs.current.turn != _WIN_LINE.get("turn"):
        _WIN_LINE["steps"] = None
        return None
    indices = _match_descs(obs, steps[0][1])
    if indices is None or not (obs.select.minCount <= len(indices) <= obs.select.maxCount):
        _WIN_LINE["steps"] = None
        return None
    _WIN_LINE["steps"] = steps[1:] or None
    _DIAG["lethal_replay"] = _DIAG.get("lethal_replay", 0) + 1
    return indices
...
def _search_choose(obs, budget_seconds: float = SEARCH_BUDGET_SECONDS) -> list[int] | None:
    ...
    H-019: 승리 종단에 도달한 라인은 별도 집계한다. 2회 이상(발견+재검증) 승리가
    확인되면 평균 투표를 건너뛰고 라인 전체를 커밋한다.
    """
    ...
    for _ in range(DETERMINIZATION_SAMPLES):
        ...
        if remaining <= 0.2:
            break
        values, wins = _search_once(obs, min(remaining, budget_seconds / DETERMINIZATION_SAMPLES + 0.4))
        ...
        for key, path in wins.items():
            ...
            if key not in win_paths or len(path) < len(win_paths[key]):
                win_paths[key] = path
    if win_votes:
        _DIAG["lethal_found"] = _DIAG.get("lethal_found", 0) + 1
        key, count = max(win_votes.items(), key=lambda kv: kv[1])
        path = win_paths[key]
        if count < 2:
            verified = _verify_win_line(obs, path, tries=2)
            _DIAG["lethal_verify_ok"] = _DIAG.get("lethal_verify_ok", 0) + verified
            count += verified
        if count >= 2:
            _WIN_LINE["turn"] = obs.current.turn
            _WIN_LINE["steps"] = list(path[1:]) or None
            _DIAG["lethal_commit"] = _DIAG.get("lethal_commit", 0) + 1
            return list(key)
    if not votes:
        return None
...
def _search_once(obs, budget_seconds: float) -> tuple[dict, dict]:
    start = _time.perf_counter()
    mi = obs.current.yourIndex
    ...
    values: dict = {}
    wins: dict = {}  # first-move key -> 승리 종단까지의 (sel, descs) 경로
    budget = SEARCH_BUDGET_NODES // DETERMINIZATION_SAMPLES
    deadline = start + budget_seconds
    heap = [(0.0, next(counter), root, ())]  # path: ((sel...), (desc...)) 스텝 튜플
    try:
        while heap and budget > 0 and _time.perf_counter() < deadline:
            _, _, ss, path = heapq.heappop(heap)
            first = list(path[0][0]) if path else None
            o = ss.observation
            st = o.current
            if st is None:
                continue
            winner = _sim_winner(st, mi)
            if winner != -1 or st.yourIndex != mi or o.select is None or not o.select.option:
                value = _value_of(st, mi)
                if winner == mi and path:
                    _DIAG["terminal_win"] = _DIAG.get("terminal_win", 0) + 1
                    key = tuple(sorted(first))
                    if key not in wins or len(path) < len(wins[key]):
                        wins[key] = path
                if winner == -1 and st.yourIndex != mi:
                    ...
                    if replied_state is not None:
                        v_reply = _value_of(replied_state, mi)
                        nxt = _greedy_self(replied, mi, deadline=deadline + 0.7)
                        nxt_state = nxt.observation.current
                        v_next = _value_of(nxt_state, mi) if nxt_state is not None else v_reply
            ...
            for sel in _candidate_selects(o, cap):
                ...
                budget -= 1
                _DIAG["nodes"] += 1
                descs = tuple(describe(o, o.select.option[i]) for i in sel)
                _is_attack = any(
                    o.select.option[i].type == OptionType.ATTACK for i in sel
                )
                try:
                    child = search_step(ss.searchId, sel)
                except Exception:  # noqa: BLE001 - 불법 시퀀스는 가지치기
                    _DIAG["step_fail"] = _DIAG.get("step_fail", 0) + 1
                    if _is_attack:
                        _DIAG["step_fail_attack"] = _DIAG.get("step_fail_attack", 0) + 1
                    continue
                if _is_attack:
                    _DIAG["step_ok_attack"] = _DIAG.get("step_ok_attack", 0) + 1
                child_path = path + ((tuple(sel), descs),)
                child_first = first if first is not None else sel
                child_state = child.observation.current
                value = _value_of(child_state, mi) if child_state is not None else float("-inf")
                if child_first is not None and value > float("-inf"):
                    key = tuple(sorted(child_first))
                    values[key] = max(values.get(key, float("-inf")), value)
                    if child_state is not None and _sim_winner(child_state, mi) == mi:
                        _DIAG["terminal_win"] = _DIAG.get("terminal_win", 0) + 1
                        if key not in wins or len(child_path) < len(wins[key]):
                            wins[key] = child_path
                heapq.heappush(heap, (-value, next(counter), child, child_path))
    finally:
        search_end()
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        try:
            _update_prize_inference(obs)
        except Exception:  # noqa: BLE001 - 추론 실패는 무추론과 동일하게 취급
            pass
        replay = _pop_win_step(obs)
        if replay is not None:
            n = len(obs.select.option)
            if all(0 <= i < n for i in replay):
                return replay

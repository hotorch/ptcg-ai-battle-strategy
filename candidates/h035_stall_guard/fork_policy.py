...
def heuristic_scores(obs):
    ...
    # H-035: 스톨 가드 — 턴 8 이후 상대가 프라이즈 0이면 스톨 덱으로 간주하고
    # 덱 18장 이하에서 드로 채널(특성·드로 트레이너·드로 에너지)을 보존한다.
    # H-030의 마진 가드는 마진≥3이면 계속 드로를 허용하는데, 스톨 상대 20턴+
    # 게임에서는 의무 드로만으로 고갈된다(래더 실측: Crustle전 프라이즈 4-0/5-0
    # 우세 중 덱아웃 2패, h024b 표본에서도 스톨류 덱아웃 6/14). safe_draws
    # 자체는 건드리지 않는다 — Rare Candy·Dawn·Poffin 등 비드로 카드까지
    # 막으면 게임을 닫을 공격 라인 구축이 멈춘다.
    opp_prizes_taken = 6 - len(op_state.prize)
    stall_lock = (state.turn >= 8 and opp_prizes_taken == 0
                  and not can_win_this_turn and deck_count <= 18)
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # H-030: 저덱 드로 밴 — MAIN ABILITY 가드와 달리 ACTIVATE 프롬프트(진화
            # 시 Psychic Draw 포함)는 무가드였음(래더 deck-out 자멸 14패의 발화 지점).
            # safe_draws의 can_win_this_turn 탈출구는 쓰지 않는다: 잔여 프라이즈가
            # 적으면 포크의 킬 추정이 낙관 오판으로 밴을 무력화했고(91680994),
            # 진짜 리썰용 드로는 커밋 라인 재생 경로가 별도로 처리한다(래더 실측
            # NO 10/10 전승). 대상: 덱의 3장 드로 특성 4종 전부.
            if (context == SelectContext.ACTIVATE
                    and select.contextCard is not None
                    and select.contextCard.id in (Kadabra, Alakazam, Dudunsparce, Fezandipiti_ex)
                    and (deck_count - my_prize_count - 1 < 3 or stall_lock)):
                score = -1
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if cid == Buddy_Buddy_Poffin:
                    ...
                    if safe_draws < 1 or overdraw or stall_lock: score = -1
                    ...
                    score = W["hilda"] if (safe_draws >= 2 and not overdraw and not stall_lock) else -1
                elif cid == Dawn:
                    if overdraw: score = -1
                    ...
                    if safe_draws < 6 or stall_lock: score = -1
            ...
            if card.id == Lucky_Helmet:
                ...
                if card.id == Telepath_Psychic_Energy and (safe_draws < 2 or stall_lock) and score > 0: score = -1
            elif card.id == Enriching_Energy:
                ...
                if (safe_draws < 4 or stall_lock) and score > 0: score = -1
            ...
            if card.id == Dudunsparce:
                if len(my_field) <= 1: score = -1
                elif safe_draws < 3 or overdraw or stall_lock: score = -1
                ...
                score = W["ability_fez"] if (need_fez and safe_draws >= 3 and not stall_lock) else -1
            elif card.id == Fan_Rotom:

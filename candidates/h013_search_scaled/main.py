...
SEARCH_BUDGET_NODES = 1200
SEARCH_BUDGET_SECONDS = 3.5
DETERMINIZATION_SAMPLES = 3  # 숨은 정보 샘플 수. 투표로 단일 샘플 노이즈를 상쇄한다.
GAME_SEARCH_SECONDS_CAP = 1200.0  # runTimeout 방어: 초과 시 남은 경기는 fallback 정책
...
def _search_choose(obs) -> list[int] | None:
    ...
    for _ in range(DETERMINIZATION_SAMPLES):
        remaining = start + SEARCH_BUDGET_SECONDS - _time.perf_counter()
        if remaining <= 0.2:
            break
        values = _search_once(obs, min(remaining, SEARCH_BUDGET_SECONDS / DETERMINIZATION_SAMPLES + 0.4))
    ...
    try:
        while heap and budget > 0 and _time.perf_counter() < deadline:
            ...
            if st.result != -1 or st.yourIndex != mi or o.select is None or not o.select.option:
                ...
                continue
            cap = 12 if o.select.context == SelectContext.MAIN else 5
